...
for i = 1:filesNum
    for j=1:nodeNum
        ...
        file2 = ['D:\professional_install\matlab\bin\project_files\relative_position_chen\log', num2str(i),'\flooding_Tx', num2str(j), '.log'];
        fid2 = fopen(file2);
...
for i = 1:filesNum
    for j=1:nodeNum
        ...
        file4 = ['D:\professional_install\matlab\bin\project_files\relative_position_chen\log', num2str(i),'\random_Tx', num2str(j), '.log'];
        fid4 = fopen(file4);
...
for i = 1:filesNum
    for j=1:nodeNum
        ...
        file6 = ['D:\professional_install\matlab\bin\project_files\relative_position_chen\log', num2str(i),'\relative_pos_Tx', num2str(j), '.log'];
        fid6 = fopen(file6);
...
for i = 1:filesNum
    for j=1:nodeNum
        ...
        file8 = ['D:\professional_install\matlab\bin\project_files\relative_position_chen\log', num2str(i),'\mobility_gradient_Tx', num2str(j), '.log'];
        fid8 = fopen(file8);
        C8 = textscan(fid8, '%f%f%f');          %%其中fid为fopen命令返回的文件标识符，format实际上就是一个字符串变量，表示读取数据及数据转换的规则。
        fclose(fid8);
        dataTx4 = [C8{3}];                         %%取出第3个元素
        result4(j) = (length(dataRx4) / length(dataTx4) * 100) + result4(j); 
    end
end
...
for i = 1:filesNum
    for j=1:nodeNum
        ...
        fclose(fid8);
        dataTx4 = [C8{3}];                         %%取出第3个元素
        Variance4(j) = ((length(dataRx4) / length(dataTx4) * 100) - result4(j))^2 + Variance4(j);
    end
end
Variance4 = sqrt(Variance4)./filesNum*zValue;
errorbar(all_nodeNums(:,1), result4(:,1),Variance4, '--sr','LineWidth',linearWidth);
legend('Flooding', 'Random', 'FGOR based on relative position model', 'FGOR based on mobility gradient model','Location','southwest');
