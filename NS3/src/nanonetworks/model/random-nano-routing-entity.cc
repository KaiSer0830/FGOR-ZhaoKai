...
void RandomNanoRoutingEntity::SendPacket (Ptr<Packet> p)
{
	if(GetDevice()->GetEnergyCapacity() >= GetDevice()->GetMinSatisfidSendEnergy()) {			//节点能量满足最小发送能量
		...
		if (neighbors.size() != 0) {
			...
			std::cout << Simulator::Now().GetSeconds() << " " << "GetNode()->GetId():" << GetDevice ()->GetNode()->GetId() << "   " << GetDevice ()->GetPhy()->GetMobility()->GetPosition() << "   " << "neighbors.size ():" << neighbors.size () << std::endl;
			...
			Simulator::Schedule (Seconds (GetDevice ()->reSendTimeIntervalOfNeighbors), &RandomNanoRoutingEntity::SendPacket, this, p);			//0.1s
		}
	} else {	 		//没有能量,间隔reSendTimeInterval之后再次调用该节点的mac协议发送
		std::cout << GetDevice()->GetNode()->GetId() << "   " << "SendPacket no energy to send" << std::endl;
		Simulator::Schedule (Seconds (GetDevice ()->reSendTimeIntervalOfEnergyCap), &RandomNanoRoutingEntity::SendPacket, this, p);			//0.5s
	}
}
...
void RandomNanoRoutingEntity::ReceivePacket (Ptr<Packet> p, Ptr<SpectrumPhy> sourcePhy)
{
	...
	if (!alreadyReceived) {
		std::vector<NanoDetail> nowNeighbors;
		if (sourcePhy->GetDevice()->GetObject<SimpleNanoDevice>()->m_type == SimpleNanoDevice::NanoRouter) {		//根据源节点类型取出相应的候选节点集
			nowNeighbors = sourcePhy->GetDevice()->GetObject<SimpleNanoDevice>()->GetMac()->m_neighbors;
		} else if (sourcePhy->GetDevice()->GetObject<SimpleNanoDevice>()->m_type == SimpleNanoDevice::NanoNode) {
			nowNeighbors = sourcePhy->GetDevice()->GetObject<SimpleNanoDevice>()->GetMac()->m_neighborss;
		}
		std::vector<NanoDetail>::iterator it;
		//遍历候选节点，候选节点中有节点id跟自己相等的则接收数据包
		for (it = nowNeighbors.begin (); it != nowNeighbors.end (); it++) {
			//遍历候选节点，候选节点中有节点id跟自己相等的则接收数据包
			if((*it).detail_priority_seq == 0 && (*it).detail_id == GetDevice()->GetNode()->GetId() ) {
				if((*it).detail_type == 0) {				//网关节点接收
				...
				} else if ((*it).detail_type == 1 && GetDevice()->packetExistFlag == false) {			//路由节点接收并转发
					if (sourcePhy->GetDevice()->GetObject<SimpleNanoDevice>()->m_type == SimpleNanoDevice::NanoNode) {				//源节点是纳米节点
						sourcePhy->GetDevice()->GetObject<SimpleNanoDevice>()->ConsumeEnergyReceive(GetDevice()->GetAckSize());		//发送节点消耗接收成功接收ACK应答包能量
						//先判断发送节点之前定时产生数据包的事件是否还存在，如果不存在则重新产生，如果存在则不产生
						sourcePhy->GetDevice()->GetObject<SimpleNanoDevice>()->EvendJudge();
					}
					...
					ForwardPacket(p);
				} else if ((*it).detail_type == 2 && GetDevice()->packetExistFlag == false && GetDevice()->m_energy >= GetDevice()->GetMinSatisfidForwardEnergy()){										//接收的不是网关节点，进行数据包转发
...
void RandomNanoRoutingEntity::ForwardPacket (Ptr<Packet> p)
{
	if(GetDevice()->m_type == SimpleNanoDevice::NanoNode && GetDevice()->GetEnergyCapacity() >= GetDevice()->GetMinSatisfidSendEnergy()) {			//节点能量满足最小发送能量
		...
		if (neighbors.size() != 0) {
			...
			Simulator::Schedule (Seconds (GetDevice ()->reSendTimeIntervalOfNeighbors), &RandomNanoRoutingEntity::ForwardPacket, this, p);			//0.1s
		}
		...
		Simulator::Schedule (Seconds (GetDevice ()->reSendTimeIntervalOfEnergyCap), &RandomNanoRoutingEntity::ForwardPacket, this, p);			//0.5s
		...
		if (neighbors.size () != 0) {			//判断周围是否有网关节点或者其它符合要求的纳米节点，底层函数已过滤路由节点、index值不符合要求的节点、已携带数据包的节点、剩余能量不足的节点
			std::cout << Simulator::Now().GetSeconds() << " " << "RouterForward---GetNode()->GetId():" << GetDevice ()->GetNode()->GetId() << "   " << GetDevice ()->GetPhy()->GetMobility()->GetPosition() << "   " << "neighbors.size ():" << neighbors.size () << std::endl;
			...
			if (ttl > 0) {
				...
				p->AddHeader(l3Header);
				if(neighbors.size() == 1) {
					GetDevice ()->GetMac ()->m_neighbors.at(0).detail_priority_seq = 0;				//只有一个元素时也需要设置优先级
					GetDevice()->GetMac()->Send(p);
				} else if(neighbors.size() > 1) {
					srand(time(NULL));
					int i = rand() % neighbors.size();
					GetDevice ()->GetMac ()->m_neighbors.at(i).detail_priority_seq = 0;				//随机选中的候选节点优先级最高
					GetDevice()->GetMac()->Send(p);
				}
			} else {
				std::cout << Simulator::Now().GetSeconds() << " " << "TTL expire" << "   " << "ttl:" << l3Header.GetTtl() << "   " << "GetId:" << GetDevice()->GetNode()->GetId() << "   " << "packetId:" << p->GetUid() << std::endl;
