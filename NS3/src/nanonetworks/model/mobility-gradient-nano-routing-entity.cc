...
#include "mobility-gradient-nano-routing-entity.h"
#include "ns3/log.h"
...
NS_LOG_COMPONENT_DEFINE ("MobilityGradientNanoRoutingEntity");

namespace ns3 {


NS_OBJECT_ENSURE_REGISTERED (MobilityGradientNanoRoutingEntity);

TypeId MobilityGradientNanoRoutingEntity::GetTypeId (void)
{
	static TypeId tid = TypeId ("ns3::MobilityGradientNanoRoutingEntity").SetParent<Object> ();
	return tid;
}


MobilityGradientNanoRoutingEntity::MobilityGradientNanoRoutingEntity ()
{
	SetDevice(0);
...
void  MobilityGradientNanoRoutingEntity::DoDispose (void)
{
  SetDevice (0);
}

void MobilityGradientNanoRoutingEntity::SendPacket (Ptr<Packet> p)					//从MessageProcessUnit::CreteMessage中的m_device->SendPacket (p)进入
{
	if(GetDevice()->GetEnergyCapacity() >= GetDevice()->GetMinSatisfidSendEnergy()) {			//节点能量满足最小发送能量
		...
		GetDevice ()->GetMac ()->CheckForMobilityGradientNeighbors(p);
		...
		if (neighbors.size () != 0) {			//判断周围是否有网关节点或者其它符合要求的纳米节点，底层函数已过滤路由节点、index值不符合要求的节点、已携带数据包的节点、剩余能量不足的节点
			std::cout << Simulator::Now().GetSeconds() << " " << "send---GetNode()->GetId():" << GetDevice ()->GetNode()->GetId() << "   " << GetDevice ()->GetPhy()->GetMobility()->GetPosition() << "   " << "neighbors.size ():" << neighbors.size () << "   " << "index:" << GetDevice()->index << "   " << "packetId:" << p->GetUid() << std::endl;
			...
			for (it = neighbors.begin (); it != neighbors.end (); it++) {
				std::cout << Simulator::Now().GetSeconds() << " " << "neighbor.id:" << (*it).detail_id << "   " << (*it).detail_phy->GetMobility()->GetPosition() << "   " << "detail_average_index:" << (*it).detail_average_index << "   " << "packetId:" << p->GetUid() << "   " << "ttl:" << l3Header.GetTtl() << std::endl;
				//std::cout << Simulator::Now().GetSeconds() << " " << "neighbor.id:" << (*it).detail_id << "   " << (*it).detail_phy->GetMobility()->GetPosition() << "   " << "detail_index:" << (*it).detail_index << "   " << "packetId:" << p->GetUid() << std::endl;
				if((*it).detail_type == 0)	{		//如果是网关节点,将数据包发送给网关节点，其它节点不接收
					...
					if (GetDevice()->averageIndex < (*it).detail_average_index) {
			...
			Simulator::Schedule (Seconds (GetDevice ()->reSendTimeIntervalOfNeighbors), &MobilityGradientNanoRoutingEntity::SendPacket, this, p);			//0.1s
		}
	} else {
		std::cout << "SendPacket no energy to send" << std::endl;
		Simulator::Schedule (Seconds (GetDevice ()->reSendTimeIntervalOfEnergyCap), &MobilityGradientNanoRoutingEntity::SendPacket, this, p);			//0.5s
	}
}

void MobilityGradientNanoRoutingEntity::SendGatewaytestPacket (Ptr<Packet> p)		//目的节点发送探测包,数据包添加序列头和路由头
{
	GetDevice()->GetMac()->CheckForNeighbors(p);
	if (GetDevice()->GetMac()->m_neighbors.size() != 0) {				//给路由节点范围内所有节点发送探测包
		NanoL3Header l3Header;
		SenderTypeTag tag;
		tag.type = 3;				//在nano-routing-entity.h中定义，大小为4字节，为4代表为探测数据包
		l3Header.SetTtl(1);
		l3Header.SetPacketId(p->GetUid());
...
void MobilityGradientNanoRoutingEntity::ReceivePacket (Ptr<Packet> p, Ptr<SpectrumPhy> sourcePhy)				//从ts-ook-based-nano-spectrum-phy.cc中的GetL3 ()->ReceivePacket (p)跳转过来的
{
	SenderTypeTag tag;
	...
	if (!alreadyReceived) {			//		//如果没有接收过该数据包，则处理该数据包
		if (GetDevice()->m_type == SimpleNanoDevice::NanoInterface  && tag.type == 0) {
		...
		} else if (GetDevice()->m_type == SimpleNanoDevice::NanoRouter && sourcePhy->GetDevice()->GetObject<SimpleNanoDevice>()->m_type != SimpleNanoDevice::NanoInterface) {			//路由节点接收并转发
			...
			if (sourcePhy->GetDevice()->GetObject<SimpleNanoDevice>()->m_type == SimpleNanoDevice::NanoNode) {
				...
				if ((sourcePhy->GetDevice()->GetObject<SimpleNanoDevice>()->GetNode()->GetId() < 13 && sourcePhy->GetDevice()->GetObject<SimpleNanoDevice>()->GetNode()->GetId() > GetDevice()->GetNode()->GetId()) ||
						(sourcePhy->GetDevice()->GetObject<SimpleNanoDevice>()->GetNode()->GetId() > 12 && sourcePhy->GetDevice()->GetObject<SimpleNanoDevice>()->GetNode()->GetId() < GetDevice()->GetNode()->GetId())) {
					p->AddHeader(l3Header);
					UpdateReceivedPacketId(p->GetUid());			//将数据包id记录队列，防止重复接收
					sourcePhy->GetDevice()->GetObject<SimpleNanoDevice>()->packetExistFlag = false;			//自身数据包被别的节点接收，标志位置为true，节点此时没有数据包，可以进行转发别的节点数据包
					GetDevice ()->packetExistFlag = true;			//接收节点接收了转发数据包，该节点该无法创建新的数据包，也无法再接收转发数据包，标志位置为false
			...
			if (!alreadyReceived) {
				if (l3Header.GetSource() == 0 && tag.type == 3 && GetDevice()->m_type == SimpleNanoDevice::NanoNode) {			//Etag与index置0函数，发送节点是网关节点并且接收数据包的节点是纳米节点且Etag还未置0
					sourcePhy->GetDevice()->GetObject<SimpleNanoDevice>()->ConsumeEnergyReceive(GetDevice()->GetAckSize());		//发送节点消耗接收成功接收ACK应答包能量
					GetDevice()->index = l3Header.GetIndex();		//接收探测包消耗能量写在了channel函数中
				} else if (l3Header.GetSource() != 0 && tag.type == 2 && GetDevice()->m_type == SimpleNanoDevice::NanoNode) {		//发送节点是普通纳米节点，自身没有数据包，将数据包转发给index比自己小的节点
					GetDevice()->ConsumeEnergyReceive(GetDevice()->GetPacketSize());				//范围内所有节点都需要先消耗能量接收数据包
					//筛选出index值、剩余能量与packetExistFlag符合要求的节点进行接收
					if (sourcePhy->GetDevice()->GetObject<SimpleNanoDevice>()->averageIndex < GetDevice()->averageIndex && GetDevice()->packetExistFlag == false && GetDevice()->m_energy >= GetDevice()->GetMinSatisfidForwardEnergy()) {
						...
						for (it = nowNeighbors.begin (); it != nowNeighbors.end (); it++) {
							if ((*it).detail_id == GetDevice()->GetNode()->GetId() && (*it).detail_priority_seq == 0) {			//按优先级退避转发，如果优先级最高，直接发送
								...
								GetDevice ()->packetExistFlag = true;			//接收节点接收了转发数据包，该节点该无法创建新的数据包，也无法再接收转发数据包，标志位置为false
								std::cout << Simulator::Now().GetSeconds() << " " << "forward Node-Node" << "   " << "packetId:" << p->GetUid() << "   " << "ttl:" << l3Header.GetTtl() << "   " << "sourcePhyId:" << sourcePhy->GetDevice()->GetNode()->GetId() << " ------------------------> " << GetDevice()->GetNode()->GetId() << std::endl;
								ForwardPacket(p);
...
void MobilityGradientNanoRoutingEntity::ForwardPacket (Ptr<Packet> p)
{
	if(GetDevice()->m_type == SimpleNanoDevice::NanoNode && GetDevice()->GetEnergyCapacity() >= GetDevice()->GetMinSatisfidSendEnergy()) {			//纳米节点能量满足最小发送能量
		...
		if (neighbors.size () != 0) {			//判断周围是否有网关节点或者其它符合要求的纳米节点，底层函数已过滤路由节点、index值不符合要求的节点、已携带数据包的节点、剩余能量不足的节点
			std::cout << Simulator::Now().GetSeconds() << " " << "NodeForward---GetNode()->GetId():" << GetDevice ()->GetNode()->GetId() << "   " << GetDevice ()->GetPhy()->GetMobility()->GetPosition() << "   " << "neighbors.size ():" << neighbors.size () << "   " << "index:" << GetDevice()->index << std::endl;
			GetDevice()->ConsumeEnergyReceive(GetDevice()->GetTestSize() * neighbors.size());				//发送节点收到所有候选节点的探测数据包，需要消耗能量
			NanoL3Header l3Header;
			...
			if (ttl > 0) {
				...
				std::vector<NanoDetail>::iterator it;
				for (it = neighbors.begin (); it != neighbors.end (); it++) {
					std::cout << Simulator::Now().GetSeconds() << " " << "neighbor.id:" << (*it).detail_id << "   " << (*it).detail_phy->GetMobility()->GetPosition() << "   " << "detail_average_index:" << (*it).detail_average_index << "   " << "packetId:" << p->GetUid() << "   " << "ttl:" << l3Header.GetTtl() << "   " << "priority:" << (*it).detail_priority_seq << std::endl;
				...
				if (localFlag == false && newNeighbors.size () != 0) {	 		//将数据包转发给邻居节点
					...
					p->AddPacketTag (tag);
					GetDevice()->GetMac()->Send(p);
				} else if (localFlag == false && newNeighbors.size () == 0) {
					Simulator::Schedule (Seconds (GetDevice ()->reSendTimeIntervalOfNeighbors), &MobilityGradientNanoRoutingEntity::ForwardPacket, this, p);			//0.1s
				}
			} else {
	...
	} else if (GetDevice()->m_type == SimpleNanoDevice::NanoNode && GetDevice()->GetEnergyCapacity() < GetDevice()->GetMinSatisfidSendEnergy()){			//纳米节点转发
		std::cout << GetDevice()->GetNode()->GetId() << "ForwardPacket " << GetDevice ()->GetNode()->GetId() << " no energy to send" << std::endl;
		Simulator::Schedule (Seconds (GetDevice ()->reSendTimeIntervalOfEnergyCap), &MobilityGradientNanoRoutingEntity::ForwardPacket, this, p);			//0.5s
		...
		if (neighbors.size () != 0) {			//判断周围是否有网关节点或者其它符合要求的纳米节点，底层函数已过滤路由节点、index值不符合要求的节点、已携带数据包的节点、剩余能量不足的节点
			...
			if (ttl > 0) {
				...
				SenderTypeTag tag1;		//在nano-routing-entity.h中定义，大小为4字节，标记数据包的类型
				//GetDevice()->ConsumeEnergySend(GetDevice()->GetPacketSize());				//节点消耗发送数据包的能量，能量是否足够判断已在候选节选择点中判断过
				std::vector<NanoDetail>::iterator it;

				for (it = neighbors.begin (); it != neighbors.end (); it++) {
					std::cout << Simulator::Now().GetSeconds() << " " << "neighbor.id:" << (*it).detail_id << "   " << "detail_average_index:" << (*it).detail_average_index << "   " << "packetId:" << p->GetUid() << "   " << "ttl:" << l3Header.GetTtl() << std::endl;
					if(((*it).detail_type == 1) || ((*it).detail_type == 0)) {		//如果是网关节点,将数据包发送给网关节点，其它节点不接收
						if ((*it).detail_type == 0) {
							tag1.type = 0;
						} else if((*it).detail_type == 1) {
							tag1.type = 1;
						...
						p->AddPacketTag(tag1);				//tag是放入packet中的m_packetTagList，不读入数据包长度
						GetDevice()->GetMac()->Send(p);
						break;
					}
				}
...
void MobilityGradientNanoRoutingEntity::SetReceivedPacketListDim (int m)
{
	NS_LOG_FUNCTION (this);
	m_receivedPacketListDim = m;
}

void MobilityGradientNanoRoutingEntity::UpdateReceivedPacketId (uint32_t id)
{
	NS_LOG_FUNCTION (this);
...
bool MobilityGradientNanoRoutingEntity::CheckAmongReceivedPacket (uint32_t id)
{
  NS_LOG_FUNCTION (this);
