...
void MessageProcessUnit::CreteMessage ()				//从主函数进入
{
	...
	if(GetDevice()->packetExistFlag == false && Simulator::Now().GetSeconds() < 15) {  //前一个数据包是否被接收完毕，一个节点只能拥有一个数据包，自己产生的或者是转发别人的
...
void MessageProcessUnit::CreateGatewaytestMessage ()			//网关节点创建探测数据包，在主程序调用
{
	...
	GetDevice()->index++;
	//std::cout << "index: " << GetDevice()->index << std::endl;
	if (GetDevice()->index == 512) {
		GetDevice()->index = 0;
	}
    ...
    Simulator::Schedule (Seconds (GetDevice ()->GetInterarrivalTestTime()), &MessageProcessUnit::CreateGatewaytestMessage, this);		//每隔0.1秒目的节点发送一次探测数据包
}

void MessageProcessUnit::ComputeAverageIndex ()			//计算邻居节点的平均index
{
	GetDevice()->ConsumeEnergySend(GetDevice()->GetTestSize());
	GetDevice ()->GetMac ()->CheckForMobilityGradientNeighborss();
	...
	if (neighbors.size() != 0)
	{
		...
		{
			//std::cout << "neighborId:" << (*it).detail_id << "   " << "Index:" << (*it).detail_index << "   " << std::endl;
			GetDevice()->ConsumeEnergyReceive(GetDevice()->GetTestSize());
			sum = sum + (*it).detail_index;
		}
		average = sum * 1.0 / neighbors.size();					//求周围节点的平均index值
		GetDevice()->averageIndex = (average - GetDevice()->averageIndex) * 1.0;			//节点平均index值的周期变化率，用来表示节点运动的相对方向
		//std::cout << "Id:" << GetDevice()->GetNode()->GetId() << "   " << "averagerIndex:" << GetDevice()->averageIndex << "   " << std::endl;
	}

	Simulator::Schedule (Seconds (0.1), &MessageProcessUnit::ComputeAverageIndex, this);			//每隔0.1秒目的节点发送一次探测数据包
}
