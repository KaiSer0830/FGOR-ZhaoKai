...
#include "ns3/mobility-gradient-nano-routing-entity.h"
#include<typeinfo>
...
void NanoSpectrumChannel::StartTx(Ptr<SpectrumSignalParameters> txParams) {
	...
	for (PhyList::const_iterator rxPhyIterator = m_phyList.begin(); rxPhyIterator != m_phyList.end(); ++rxPhyIterator) {
		if ((*rxPhyIterator) != txParams->txPhy) {
			...
			if (fabs(txRange - distance) < 0.00001 || txRange > distance) {			//double类型比较会产生精度问题
...
std::vector<NanoDetail> NanoSpectrumChannel::GetNeighbors(Ptr<SpectrumPhy> phy, Ptr<Packet> p) {				//网关节点与路由节点发送探测包时的获取邻居节点函数,获取的邻居节点类型包括index、节点id、节点类型
...
//			if (phy->GetDevice()->GetObject<SimpleNanoDevice>()->m_type  == SimpleNanoDevice::NanoRouter && (*rxPhyIterator)->GetDevice()->GetObject<SimpleNanoDevice>()->m_type  == SimpleNanoDevice::NanoRouter) {	//路由节点选择候选转发节点时过滤掉纳米节点
//				std::cout << phy->GetDevice()->GetNode()->GetId() << "   " << (*rxPhyIterator)->GetDevice()->GetNode()->GetId() << "   " << distance << "   " << txRange << "   " << bool(fabs(txRange - distance) < 0.00001) << std::endl;
//			}
			if (fabs(txRange - distance) < 0.00001 || txRange > distance) {					//根据距离判断邻居节点,double类型比较会产生精度问题
				//所有接收网关节点探测数据包的节点都消耗接收能量

				if(phy->GetDevice()->GetObject<SimpleNanoDevice>()->GetL3()->GetObject<OpportunisticNanoRoutingEntity>() != 0) {		//只有机会路由协议才执行，其它路由协议不执行
					if (phy->GetDevice()->GetObject<SimpleNanoDevice>()->m_type  == SimpleNanoDevice::NanoInterface && (*rxPhyIterator)->GetDevice()->GetObject<SimpleNanoDevice>()->m_type  == SimpleNanoDevice::NanoNode) {	//只有纳米节点才消耗更新Index值的能量
					...
					if (phy->GetDevice()->GetObject<SimpleNanoDevice>()->m_type  == SimpleNanoDevice::NanoInterface && (*rxPhyIterator)->GetDevice()->GetObject<SimpleNanoDevice>()->m_type  == SimpleNanoDevice::NanoRouter) {
						continue;			//网关节点选择候选转发节点时过滤掉路由节点,主要用于Index值更新,路由节点不需要更新
					}
					if (phy->GetDevice()->GetObject<SimpleNanoDevice>()->m_type  == SimpleNanoDevice::NanoRouter && (*rxPhyIterator)->GetDevice()->GetObject<SimpleNanoDevice>()->m_type  == SimpleNanoDevice::NanoNode) {		//路由节点选择候选转发节点时过滤掉纳米节点
						continue;
					}
					if ((phy->GetDevice()->GetNode()->GetId() > 0 && phy->GetDevice()->GetNode()->GetId() <= 12 && (*rxPhyIterator)->GetDevice()->GetNode()->GetId() > phy->GetDevice()->GetNode()->GetId()) ||		//路由节点选择候选转发节点时设置转发方向
							(phy->GetDevice()->GetNode()->GetId() > 12 && phy->GetDevice()->GetNode()->GetId() < 25 && (*rxPhyIterator)->GetDevice()->GetNode()->GetId() < phy->GetDevice()->GetNode()->GetId() &&
									(*rxPhyIterator)->GetDevice()->GetObject<SimpleNanoDevice>()->m_type  != SimpleNanoDevice::NanoInterface)) {		//路由节点选择候选转发节点时过滤掉纳米节点
						continue;
					}
				}

				if (phy->GetDevice()->GetObject<SimpleNanoDevice>()->m_type  == SimpleNanoDevice::NanoRouter && (*rxPhyIterator)->GetDevice()->GetObject<SimpleNanoDevice>()->packetExistFlag) {		//路由节点已经有数据包时过滤
					continue;
				}

				if ((*rxPhyIterator)->GetDevice()->GetObject<SimpleNanoDevice>()->m_type  != SimpleNanoDevice::NanoInterface) {			//过滤时不包含纳米网关
					if((*rxPhyIterator)->GetDevice()->GetObject<SimpleNanoDevice>()->GetL3()->GetObject<FloodingNanoRoutingEntity>() != 0) {			//泛洪路由协议中过滤已接收过该数据包的节点
					...
					} else if ((*rxPhyIterator)->GetDevice()->GetObject<SimpleNanoDevice>()->GetL3()->GetObject<RandomNanoRoutingEntity>() != 0) {			//随机路由协议中过滤已接收过该数据包的节点
	...
	//std::cout << Simulator::Now().GetSeconds() << "   " << phy->GetDevice()->GetNode()->GetId() << "   " << neighbors.size() << "   " << std::endl;
	return neighbors;
}
...
std::vector<NanoDetail> NanoSpectrumChannel::GetNeighborss(Ptr<SpectrumPhy> phy, Ptr<Packet> p) {				//泛洪与随机路由协议中普通纳米节点获取邻居节点函数，获取的邻居节点类型包括index、节点id、节点类型
	...
	for (PhyList::const_iterator rxPhyIterator = m_phyList.begin(); rxPhyIterator != m_phyList.end(); ++rxPhyIterator) {
		if ((*rxPhyIterator) != phy) {
			...
			if (fabs(txRange - distance) < 0.00001 || txRange > distance) {					//根据距离判断邻居节点,double类型比较会产生精度问题
				if(phy->GetDevice()->GetObject<SimpleNanoDevice>()->m_type  == SimpleNanoDevice::NanoNode && (*rxPhyIterator)->GetDevice()->GetObject<SimpleNanoDevice>()->m_type  == SimpleNanoDevice::NanoNode) {		//根据条件过滤不符合条件的普通纳米节点
...
std::vector<NanoDetail> NanoSpectrumChannel::GetOpportNeighborss(Ptr<SpectrumPhy> phy, Ptr<Packet> p) {				//循环机会路由协议中普通纳米节点获取邻居节点函数，获取的邻居节点类型包括index、节点id、节点类型、剩余能量和优先级
	...
	for (PhyList::const_iterator rxPhyIterator = m_phyList.begin(); rxPhyIterator != m_phyList.end(); ++rxPhyIterator) {
		if ((*rxPhyIterator) != phy) {
			...
			if (fabs(txRange - distance) < 0.00001 || txRange > distance) {					//根据距离判断邻居节点,double类型比较会产生精度问题
				if(phy->GetDevice()->GetObject<SimpleNanoDevice>()->m_type  == SimpleNanoDevice::NanoNode && (*rxPhyIterator)->GetDevice()->GetObject<SimpleNanoDevice>()->m_type  == SimpleNanoDevice::NanoNode) {
					//所有候选节点都需要消耗接收探测数据包的能量,除网关与路由节点外
					(*rxPhyIterator)->GetDevice()->GetObject<SimpleNanoDevice>()->ConsumeEnergyReceive((*rxPhyIterator)->GetDevice()->GetObject<SimpleNanoDevice>()->GetTestSize());
				...
				} else if((*rxPhyIterator)->GetDevice()->GetObject<SimpleNanoDevice>()->m_type  != SimpleNanoDevice::NanoNode) {
					neighbor.detail_priority_value = 0;
				}
				neighbor.detail_id = (*rxPhyIterator)->GetDevice()->GetNode()->GetId();
				neighbor.detail_phy = (*rxPhyIterator);
				if ((*rxPhyIterator)->GetDevice()->GetObject<SimpleNanoDevice>()->m_type == SimpleNanoDevice::NanoNode)			//将邻居节点进行分类
...
std::vector<NanoDetail> NanoSpectrumChannel::GetMobilityGradientNeighbors(Ptr<SpectrumPhy> phy, Ptr<Packet> p) {				//移动感知路由协议中普通纳米节点获取邻居节点函数，获取的邻居节点类型包括averageIndex、节点id、节点类型、剩余能量和优先级
	NS_LOG_FUNCTION(this);
		...
		if ((*rxPhyIterator) != phy) {
			...
			if (fabs(txRange - distance) < 0.00001 || txRange > distance) {					//根据距离判断邻居节点,double类型比较会产生精度问题
				if(phy->GetDevice()->GetObject<SimpleNanoDevice>()->m_type  == SimpleNanoDevice::NanoNode && (*rxPhyIterator)->GetDevice()->GetObject<SimpleNanoDevice>()->m_type  == SimpleNanoDevice::NanoNode) {
					...
					//过滤邻居节点中index值不符合要求的，所有过滤条件在路由协议接收时都要重新判断一遍，因为这里筛选只是统计邻居节点数目，不决定具体接收节点
					if((phy->GetDevice()->GetObject<SimpleNanoDevice>()->averageIndex >= (*rxPhyIterator)->GetDevice()->GetObject<SimpleNanoDevice>()->averageIndex)) {
						continue;
					}
					//过滤邻居节点中剩余能量不符合转发要求的
					if(((*rxPhyIterator)->GetDevice()->GetObject<SimpleNanoDevice>()->m_energy) < (phy->GetDevice()->GetObject<SimpleNanoDevice>()->GetMinSatisfidForwardEnergy())) {
					...
					//过滤邻居节点中已接收过该数据包的节点
					if((*rxPhyIterator)->GetDevice()->GetObject<SimpleNanoDevice>()->GetL3()->GetObject<MobilityGradientNanoRoutingEntity>()->CheckAmongReceivedPacket(p->GetUid())) {
						continue;
					}
				}
				...
				if((*rxPhyIterator)->GetDevice()->GetObject<SimpleNanoDevice>()->m_type  == SimpleNanoDevice::NanoNode) {
					...
					neighbor.detail_priority_value = 					//计算候选节点的转发优先级
							0.5 * (*rxPhyIterator)->GetDevice()->GetObject<SimpleNanoDevice>()->m_energy +
							0.5 * abs(phy->GetDevice()->GetObject<SimpleNanoDevice>()->averageIndex - (*rxPhyIterator)->GetDevice()->GetObject<SimpleNanoDevice>()->averageIndex) +
							0.0001 * (*rxPhyIterator)->GetDevice()->GetNode()->GetId();				//加节点id是为了降低优先级一致出现的概率
				} else if((*rxPhyIterator)->GetDevice()->GetObject<SimpleNanoDevice>()->m_type  != SimpleNanoDevice::NanoNode) {
					neighbor.detail_priority_value = 0;
				}
				neighbor.detail_average_index = (*rxPhyIterator)->GetDevice()->GetObject<SimpleNanoDevice>()->averageIndex;
				neighbor.detail_id = (*rxPhyIterator)->GetDevice()->GetNode()->GetId();
				neighbor.detail_phy = (*rxPhyIterator);
				if ((*rxPhyIterator)->GetDevice()->GetObject<SimpleNanoDevice>()->m_type == SimpleNanoDevice::NanoNode)			//将邻居节点进行分类
					neighbor.detail_type = 2;
...
std::vector<NanoDetail> NanoSpectrumChannel::GetMobilityGradientNeighborss(Ptr<SpectrumPhy> phy) {				//移动感知模型获取邻居节点计算平均index值，获取的邻居节点类型包括index、节点id、节点类型、剩余能量和优先级
