...
namespace ns3 {
	...
	void NanoMacEntity::CheckForMobilityGradientNeighbors(Ptr<Packet> p) {				//移动感知模型获取邻居节点计算平均index值
			NS_LOG_FUNCTION(this);
			m_neighborss.clear();
			Ptr<NanoSpectrumPhy> phy = GetDevice()->GetPhy();
			Ptr<NanoSpectrumChannel> channel = phy->GetChannel()->GetObject<NanoSpectrumChannel>();
			m_neighborss = channel->GetMobilityGradientNeighbors(phy, p);
	}

	void NanoMacEntity::CheckForMobilityGradientNeighborss() {				//移动感知模型获取邻居节点计算平均index值
			NS_LOG_FUNCTION(this);
			m_neighborss.clear();
			Ptr<NanoSpectrumPhy> phy = GetDevice()->GetPhy();
			Ptr<NanoSpectrumChannel> channel = phy->GetChannel()->GetObject<NanoSpectrumChannel>();
			m_neighborss = channel->GetMobilityGradientNeighborss(phy);
	}
