...
class NanoSpectrumChannel : public SpectrumChannel
{
	...
	public:
		...
		std::vector<NanoDetail> GetNeighbors(Ptr<SpectrumPhy> phy, Ptr<Packet> p);						//网关和路由节点发送探测包时的获取邻居节点函数
		...
		std::vector<NanoDetail> GetMobilityGradientNeighbors (Ptr<SpectrumPhy> phy, Ptr<Packet> p);			//flow-guided,移动感知模型获取邻居节点
		std::vector<NanoDetail> GetMobilityGradientNeighborss (Ptr<SpectrumPhy> phy);					//flow-guided,移动感知模型获取邻居节点计算平均index值
