	...
	class SimpleNanoDevice : public NetDevice
	{
		public:
			...
			*/
			...
			bool GetEtag();
			double GetMinSatisfidSendEnergy();						//设置最小发送能量
			double GetMinSatisfidForwardEnergy();					//设置最小转发能量
		...
		public:
			...
			double averageIndex;		//flow-guided,节点周围邻居节点计算得到的平均index
			...
			double reSendTimeIntervalOfNeighbors;					//flow-guided,发送节点重新发送检测邻居节点的时间间隔
			double reSendTimeIntervalOfEnergyCap;					//flow-guided,发送节点捕获能量后重新发送数据包的时间间隔
			double errorTime;				//flow-guided,误差时间
