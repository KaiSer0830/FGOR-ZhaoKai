...
void OpportunisticNanoRoutingEntity::SendPacket (Ptr<Packet> p)					//从MessageProcessUnit::CreteMessage中的m_device->SendPacket (p)进入
{
	if(GetDevice()->GetEnergyCapacity() >= GetDevice()->GetMinSatisfidSendEnergy()) {			//节点能量满足最小发送能量
		...
		if (neighbors.size () != 0) {			//判断周围是否有网关节点或者其它符合要求的纳米节点，底层函数已过滤路由节点、index值不符合要求的节点、已携带数据包的节点、剩余能量不足的节点
			std::cout << Simulator::Now().GetSeconds() << " " << "send---GetNode()->GetId():" << GetDevice ()->GetNode()->GetId() << "   " << GetDevice ()->GetPhy()->GetMobility()->GetPosition() << "   " << "neighbors.size ():" << neighbors.size () << "   " << "index:" << GetDevice()->index << std::endl;
			...
			Simulator::Schedule (Seconds (GetDevice ()->reSendTimeIntervalOfNeighbors), &OpportunisticNanoRoutingEntity::SendPacket, this, p);			//0.1s
		}
	} else {
		std::cout << "SendPacket no energy to send" << std::endl;
		Simulator::Schedule (Seconds (GetDevice ()->reSendTimeIntervalOfEnergyCap), &OpportunisticNanoRoutingEntity::SendPacket, this, p);			//0.5s
	}
}
...
void OpportunisticNanoRoutingEntity::ForwardPacket (Ptr<Packet> p)
{
	if(GetDevice()->m_type == SimpleNanoDevice::NanoNode && GetDevice()->GetEnergyCapacity() >= GetDevice()->GetMinSatisfidSendEnergy()) {			//纳米节点能量满足最小发送能量
		...
		if (neighbors.size () != 0) {			//判断周围是否有网关节点或者其它符合要求的纳米节点，底层函数已过滤路由节点、index值不符合要求的节点、已携带数据包的节点、剩余能量不足的节点
			...
			if (ttl > 0) {
				...
				if (localFlag == false && newNeighbors.size () != 0) {	 		//将数据包转发给邻居节点
					...
					Simulator::Schedule (Seconds (GetDevice ()->reSendTimeIntervalOfNeighbors), &OpportunisticNanoRoutingEntity::ForwardPacket, this, p);			//0.1s
				}
			} else {
		...
		Simulator::Schedule (Seconds (GetDevice ()->reSendTimeIntervalOfEnergyCap), &OpportunisticNanoRoutingEntity::ForwardPacket, this, p);			//0.5s
		...
		if (neighbors.size () != 0) {			//判断周围是否有网关节点或者其它符合要求的纳米节点，底层函数已过滤路由节点、index值不符合要求的节点、已携带数据包的节点、剩余能量不足的节点
			...
			if (ttl > 0) {
				...
				for (it = neighbors.begin (); it != neighbors.end (); it++) {
					std::cout << Simulator::Now().GetSeconds() << " " << "neighbor.id:" << (*it).detail_id << "   " << "detail_index:" << (*it).detail_index << "   " << "packetId:" << p->GetUid() << "   " << "ttl:" << l3Header.GetTtl() << std::endl;
