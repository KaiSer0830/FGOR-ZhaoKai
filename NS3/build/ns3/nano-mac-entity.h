...
//  virtual bool Sendack (Ptr<Packet> p, uint32_t from,uint32_t pri);
  ...
  void CheckForNeighbors (Ptr<Packet> p);								//网关和路由节点发送探测包时的获取邻居节点函数
  ...
  void CheckForMobilityGradientNeighbors (Ptr<Packet> p);				//移动感知模型获取邻居节点
  void CheckForMobilityGradientNeighborss ();							//移动感知模型获取邻居节点计算平均index值
